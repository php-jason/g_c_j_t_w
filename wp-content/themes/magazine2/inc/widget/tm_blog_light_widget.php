<?php
/*
Plugin Name: TM Blog Light (Home Left)
Plugin URI:
Description: This widget for display vertical blog (light or white) with small thumbnail in left area.
...
class TM_Blog_Light_Widget extends WP_Widget {

	// constructor widget class and description	
	function __construct() {
    	$widget_ops = array(
			'classname'   => 'widget_tm_category_blog_thumbs', 
			'description' => 'This widget for display vertical blog (light or white) with small thumbnail in Homepage left area.'
		);
    	parent::__construct('tmmyid_blog_light_thumbs', 'TM Blog Light (Home Left)', $widget_ops);
	}
...
?>
	...
	function widget($args, $instance) {
		...
		?>
			...
			<div <?php post_class(); ?>>
			<div class="blogin_item tm-post-<?php the_ID(); ?>">
				...
				<div class="blogin_images">
					...
					<?php } else { ?>
					<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
					<img src="<?php echo get_template_directory_uri(); ?>/images/nophoto100.png" />
					</a>
					<?php } ?>
				</div>
...
function tm_blw_register_widgets() {
	register_widget( 'TM_Blog_Light_Widget' );
}

add_action( 'widgets_init', 'tm_blw_register_widgets' );
?>
