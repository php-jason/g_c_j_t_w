<?php
/*
Plugin Name: TM Headline Gray (Sidebar)
Plugin URI:
Description: This widget for display headline post (Gray) in sidebar (center area and right area).
...
class TM_Headline_Gray_Widget extends WP_Widget {

	// constructor widget class and description	
	function __construct() {
    	$widget_ops = array(
			'classname'   => 'widget_tm_headline_gray', 
			'description' => 'This widget for display headline post (Gray) in sidebar (center area and right area).'
		);
    	parent::__construct('tmmyid_headline_gray_thumbs', 'TM Headline Gray (Sidebar)', $widget_ops);
	}
...
?>
	...
	function widget($args, $instance) {
			...
			echo "<div class='tm_sidebar_gray_headline'>";
		...
		?>
			...
			<!--<edit>-->
			<div class="tm_sidebar_headline_gray_box tm-post-<?php the_ID(); ?>">		
				<div class="tm_sidebar_headline_gray_images">
					<?php if (has_post_thumbnail()) { ?>
					<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
					<img src="<?php $image_id = get_post_thumbnail_id(); $image_url = wp_get_attachment_image_src($image_id,'img297', true); echo $image_url[0]; ?>">
					</a>
					<?php } else { ?>
					<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
...
function tm_hdgw_register_widgets() {
	register_widget( 'TM_Headline_Gray_Widget' );
}

add_action( 'widgets_init', 'tm_hdgw_register_widgets' );
?>
